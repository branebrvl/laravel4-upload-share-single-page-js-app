...
var emcOriginResponse = (function() {
	...
	var emcOriginToggle = {
		...
		eclipse: function(config) {
			...
			switch(config.toggle) {
				case 'default':
					from 	= 415;
					to 		= 66;
					config.duration = config.duration - 100;
					break;
				case 'expand':
					from 	= 66;
					to 		= 415;
		...
		horizoneclipse: function(config) {
			var to, from;
			switch(config.toggle) {
				case 'default':
					from 	= '415px';
		...
		landscape: function(config) {
			...
			emcLandscape		= document.getElementById(config.emcOriginId);
			Viper({
				object: emcLandscape,
				property: 'height',
				from: from,
		...
		nova: function(config) {
			switch(config.toggle) {
				case 'default':
				case 'remove':
					document.getElementById(config.emcOriginId+'e').setAttribute('src', 'about:blank');
...
var emcOriginIframe = (function() {
	...
	return {
		init: function() {
			...
			//emcOriginJSAd		= emcOriginDomain+'/components/com_emc_origin/assets/js/emcOrigin.js';
...
var emcOriginCreate	= (function() {
	...
	return {
		init: function(emcConfig, emcOriginParams, emcLocation) {
			...
			emcOriginDomain		= 'http://'+emcOriginParams.emcOriginDomain;
			//emcOriginJSAd		= emcOriginDomain+'/components/com_emc_origin/assets/js/emcOrigin.js'; NOT USED??
		...
		layout: function(template, emcOriginAd, emcLocation, iframe) {
			...
			switch(template) {
				...
				case 'horizoneclipse':
					//Fix for reskin overriding position
					var emcOriginCSS		= document.createElement('style'),
						emcOriginCSSHorizon	= 'body{background-position: 50% 66px !important;}';
						emcOriginCSS.id		= 'emcOriginCSS';
						emcOriginCSS.type 	= 'text/css';
						emcOriginCSS.media	= 'screen';
						if(emcOriginCSS.styleSheet) {
